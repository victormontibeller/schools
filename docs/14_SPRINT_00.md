...
- [ ] `docker compose up` deverá subir todos os serviços sem erros.
...
- [ ] Redis deverá estar acessível e funcional para cache e sessões.
- [ ] RabbitMQ deverá estar acessível e com management UI disponível.
- [ ] Celery worker deverá processar uma tarefa de teste com sucesso.
- [ ] Celery beat deverá estar configurado e agendando tarefas.
...
- [ ] Criar `Dockerfile` para a aplicação Django (Python 3.13, multi-stage build)
...
**Pendente (necessita Docker):**
- `Dockerfile`, `docker-compose.yml`, Traefik, Prometheus, Grafana, Loki
